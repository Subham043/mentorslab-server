...
import { ValidEventTestimonialIdPipe } from 'src/common/pipes/valid_event_testimonial_id.pipes';
import { FormDataRequest } from 'nestjs-form-data';
import { ValidPaginatePipe } from 'src/common/pipes/valid_paginate.pipes';
import { EventTestimonialAdminService } from '../services/event_testimonial.admin.service';
import { ValidEventIdPipe } from 'src/common/pipes/valid_event_id.pipes';

@UseGuards(AccessTokenGuard)
@Controller('event-testimonial')
export class EventTestimonialAdminController {
  constructor(private eventService: EventTestimonialAdminService) {}

  @Post(':event_id')
  @Roles('ADMIN')
  @FormDataRequest()
  async createContent(
    @Body() eventCreateDto: EventTestimonialAdminCreateDto,
    @Param('event_id', ValidEventIdPipe) event_id: number,
  ): Promise<EventTestimonialAdminGetDto> {
  ...
  @FormDataRequest()
  async updateContent(
    @Param('id', ValidEventTestimonialIdPipe) id: number,
    @Body() eventUpdateDto: EventTestimonialAdminUpdateDto,
  ): Promise<EventTestimonialAdminGetDto> {
